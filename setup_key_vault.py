...
2. uses your AWS credentials and customer master key (CMK) to encrypt three example 32-byte keys 
...
AWS_CMK_KEYID may be set as an environment variable.
...
def randomUUID():
    return bson.binary.UUID(bytes=uuid.uuid4().get_bytes())
...
print json_util.dumps(example_schema, json_options=opts)

print("Saved to example-schemas.json")
f = open("example-schemas.json", "w")
f.write(json_util.dumps([example_schema], json_options=opts))
f.close()
